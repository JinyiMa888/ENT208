...
Deno.serve(async (req) => {
  ...
  try {
    const { resumeText, jobDescription, jobTitle, style, matchAnalysis } = await req.json();
    ...
    const prompt = `你是一位专业的简历改写专家。请根据目标岗位要求，改写用户的简历。
  ...
  "changes": [
    {
      ...
      "rewritten": "改写后的文本",
      "reason": "改写原因",
      "type": "keyword_add | rephrase | new_point | remove"
    }
  ],
  ...
  "tips": ["额外的优化建议"]
    ...
    const aiResponse = await fetch(AI_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "你是专业简历改写专家。只返回JSON格式结果。" },
